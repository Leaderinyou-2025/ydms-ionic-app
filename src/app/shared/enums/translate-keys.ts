export enum TranslateKeys {
  // COMMON
  COMMON_APP_NAME = 'COMMON.app_name',
  COMMON_APP_SLOGAN = 'COMMON.app_slogan',
  COMMON_COPYRIGHT = 'COMMON.copyright',
  COMMON_ENGLISH = 'COMMON.english',
  COMMON_VIETNAMESE = 'COMMON.vietnamese',

  // NOTIFICATIONS
  NOTIFICATIONS_TITLE = 'NOTIFICATIONS.title',
  NOTIFICATIONS_CREATE_TITLE = 'NOTIFICATIONS.create_title',
  NOTIFICATIONS_CREATE_LABEL = 'NOTIFICATIONS.notification_title',
  NOTIFICATIONS_CREATE_DESCRIPTION = 'NOTIFICATIONS.notification_description',
  NOTIFICATIONS_CREATE_BODY = 'NOTIFICATIONS.notification_body',
  NOTIFICATIONS_CREATE_RECIPIENT = 'NOTIFICATIONS.recipient',
  NOTIFICATIONS_CREATE_SEND_ALL = 'NOTIFICATIONS.send_all',
  NOTIFICATIONS_CREATE_SEND_ALL_DESCRIPTION = 'NOTIFICATIONS.send_all_description',
  NOTIFICATIONS_CREATE_SEND_PARENTS = 'NOTIFICATIONS.send_parents',
  NOTIFICATIONS_CREATE_SEND_PARENTS_DESCRIPTION = 'NOTIFICATIONS.send_parents_description',
  NOTIFICATIONS_CREATE_SEND_STUDENTS = 'NOTIFICATIONS.send_students',
  NOTIFICATIONS_CREATE_SEND_STUDENTS_DESCRIPTION = 'NOTIFICATIONS.send_students_description',
  NOTIFICATIONS_CREATE_SEND_SPECIFIC_USERS = 'NOTIFICATIONS.send_specific_users',
  NOTIFICATIONS_CREATE_SEND_SPECIFIC_USERS_DESCRIPTION = 'NOTIFICATIONS.send_specific_users_description',
  NOTIFICATIONS_UNREAD = 'NOTIFICATIONS.unread',
  NOTIFICATIONS_READ = 'NOTIFICATIONS.read',
  NOTIFICATIONS_NO_UNREAD = 'NOTIFICATIONS.no_unread',
  NOTIFICATIONS_NO_READ = 'NOTIFICATIONS.no_read',
  NOTIFICATIONS_SEARCH = 'NOTIFICATIONS.search',
  NOTIFICATIONS_SEARCH_TITLE = 'NOTIFICATIONS.search_title',
  NOTIFICATIONS_SEARCH_STATUS = 'NOTIFICATIONS.search_status',
  NOTIFICATIONS_SEARCH_TYPE = 'NOTIFICATIONS.search_type',
  NOTIFICATIONS_SEARCH_TIME_RANGE = 'NOTIFICATIONS.search_time_range',
  NOTIFICATIONS_SEARCH_CUSTOM_DATE = 'NOTIFICATIONS.search_custom_date',
  NOTIFICATIONS_SEARCH_FROM_DATE = 'NOTIFICATIONS.search_from_date',
  NOTIFICATIONS_SEARCH_TO_DATE = 'NOTIFICATIONS.search_to_date',
  NOTIFICATIONS_TYPE_EMOTION = 'NOTIFICATIONS.type_emotion',
  NOTIFICATIONS_TYPE_TASK = 'NOTIFICATIONS.type_task',
  NOTIFICATIONS_TYPE_EXPERIENCE = 'NOTIFICATIONS.type_experience',
  NOTIFICATIONS_TYPE_ASSESSMENT = 'NOTIFICATIONS.type_assessment',
  NOTIFICATIONS_TYPE_OTHER = 'NOTIFICATIONS.type_other',
  NOTIFICATIONS_TIME_TODAY = 'NOTIFICATIONS.time_today',
  NOTIFICATIONS_TIME_THIS_WEEK = 'NOTIFICATIONS.time_this_week',
  NOTIFICATIONS_TIME_THIS_MONTH = 'NOTIFICATIONS.time_this_month',
  NOTIFICATIONS_TIME_CUSTOM = 'NOTIFICATIONS.time_custom',
  NOTIFICATIONS_TIME_MINUTES_AGO = 'NOTIFICATIONS.time_minutes_ago',
  NOTIFICATIONS_TIME_HOURS_AGO = 'NOTIFICATIONS.time_hours_ago',
  NOTIFICATIONS_TIME_DAYS_AGO = 'NOTIFICATIONS.time_days_ago',
  NOTIFICATIONS_NO_RESULTS = 'NOTIFICATIONS.no_results',
  NOTIFICATIONS_SENDER = 'NOTIFICATIONS.sender',
  NOTIFICATIONS_ATTACH_FILE = 'NOTIFICATIONS.attach_file',
  NOTIFICATIONS_SELECT_RECIPIENTS = 'NOTIFICATIONS.select_recipients',
  NOTIFICATIONS_SEND_TO_PARENT = 'NOTIFICATIONS.send_to_parent',
  NOTIFICATIONS_SEND_TO_PARENT_CONFIRM = 'NOTIFICATIONS.send_to_parent_confirm',
  NOTIFICATIONS_SEND_TO_PARENT_SUCCESS = 'NOTIFICATIONS.send_to_parent_success',
  NOTIFICATIONS_SEND_TO_PARENT_ERROR = 'NOTIFICATIONS.send_to_parent_error',
  NOTIFICATIONS_EMOTION_TITLE = 'NOTIFICATIONS.emotion_title',
  NOTIFICATIONS_EMOTION_DESCRIPTION = 'NOTIFICATIONS.emotion_description',
  NOTIFICATIONS_EMOTION_BODY = 'NOTIFICATIONS.emotion_body',


  // BUTTONS
  BUTTON_SAVE = 'BUTTON.save',

  // PERSONAL DIARY
  PERSONAL_DIARY_TITLE = 'PERSONAL_DIARY.title',
  PERSONAL_DIARY_TITLE_WELCOME = 'PERSONAL_DIARY.title_welcome',
  PERSONAL_DIARY_MY_ENTRIES = 'PERSONAL_DIARY.my_entries',
  PERSONAL_DIARY_SHARED_ENTRIES = 'PERSONAL_DIARY.shared_entries',
  PERSONAL_DIARY_NEW_ENTRY = 'PERSONAL_DIARY.new_entry',
  PERSONAL_DIARY_CONTENT_PLACEHOLDER = 'PERSONAL_DIARY.content_placeholder',
  PERSONAL_DIARY_EMOTION_TYPE = 'PERSONAL_DIARY.emotion_type',
  PERSONAL_DIARY_ANONYMOUS = 'PERSONAL_DIARY.anonymous',
  PERSONAL_DIARY_PUBLIC = 'PERSONAL_DIARY.public',
  PERSONAL_DIARY_SUBMIT = 'PERSONAL_DIARY.submit',
  PERSONAL_DIARY_CANCEL = 'PERSONAL_DIARY.cancel',
  PERSONAL_DIARY_SUGGESTIONS = 'PERSONAL_DIARY.suggestions',
  PERSONAL_DIARY_WITHDRAW = 'PERSONAL_DIARY.withdraw',
  PERSONAL_DIARY_CONFIRM_WITHDRAW = 'PERSONAL_DIARY.confirm_withdraw',
  PERSONAL_DIARY_REACTION_STATS = 'PERSONAL_DIARY.reaction_stats',
  PERSONAL_DIARY_REACTION_LOVE = 'PERSONAL_DIARY.reaction_love',
  PERSONAL_DIARY_REACTION_HAPPY = 'PERSONAL_DIARY.reaction_happy',
  PERSONAL_DIARY_REACTION_SAD = 'PERSONAL_DIARY.reaction_sad',
  PERSONAL_DIARY_REACTION_ANGRY = 'PERSONAL_DIARY.reaction_angry',
  PERSONAL_DIARY_TOTAL_REACTIONS = 'PERSONAL_DIARY.total_reactions',
  PERSONAL_DIARY_ANONYMOUS_USER = 'PERSONAL_DIARY.anonymous_user',
  PERSONAL_SELECT_SUGGESTION_GUIDE = 'PERSONAL_DIARY.select_suggestion_guide',

  // EMOTIONS
  EMOTION_HAPPY = 'EMOTION.happy',
  EMOTION_SAD = 'EMOTION.sad',
  EMOTION_ANGRY = 'EMOTION.angry',
  EMOTION_ANXIOUS = 'EMOTION.anxious',
  EMOTION_EXCITED = 'EMOTION.excited',
  EMOTION_TIRED = 'EMOTION.tired',
  EMOTION_CALM = 'EMOTION.calm',
  EMOTION_CONFUSED = 'EMOTION.confused',

  // AREA OF EXPERTISE
  AREA_OF_EXPERTISE_EMOTION = 'AREA_OF_EXPERTISE.emotion',
  AREA_OF_EXPERTISE_CONFLICT = 'AREA_OF_EXPERTISE.conflict',
  AREA_OF_EXPERTISE_COMMUNICATION = 'AREA_OF_EXPERTISE.communication',
  AREA_OF_EXPERTISE_DISCOVERY = 'AREA_OF_EXPERTISE.discovery',

  // ACHIEVEMENT
  BADGE_NO_ACHIEVEMENTS_YET = 'ACHIEVEMENT.badge_no_achievements_yet',

  // LOADING AND REFRESH
  PULL_TO_REFRESH = 'LOADING.pull_to_refresh',
  REFRESHING = 'LOADING.refreshing',
  LOADING_MORE_ITEMS = 'LOADING.loading_more_items',
  LOADING = 'LOADING.loading',
  MESSAGE_FEATURE_UNDER_DEVELOPMENT = 'MESSAGE.feature_under_development',

  // SHARE EXPERIENCE
  SHARE_EXPERIENCE_TITLE = 'SHARE_EXPERIENCE.title',
  SHARE_EXPERIENCE_CAPTION = 'SHARE_EXPERIENCE.caption',
  SHARE_EXPERIENCE_DETAIL_TITLE = 'SHARE_EXPERIENCE.detail_title',
  SHARE_EXPERIENCE_NO_POSTS = 'SHARE_EXPERIENCE.no_posts',
  SHARE_EXPERIENCE_STATUS_WAIT_ACCEPT = 'SHARE_EXPERIENCE.status_wait_accept',
  SHARE_EXPERIENCE_STATUS_ACCEPTED = 'SHARE_EXPERIENCE.status_accepted',
  SHARE_EXPERIENCE_STATUS_REJECTED = 'SHARE_EXPERIENCE.status_rejected',
  SHARE_EXPERIENCE_TAB_COMMUNITY = 'SHARE_EXPERIENCE.tab_community',
  SHARE_EXPERIENCE_TAB_FAMILY = 'SHARE_EXPERIENCE.tab_family',
  SHARE_EXPERIENCE_LIKE_COUNT = 'SHARE_EXPERIENCE.like_count',
  SHARE_EXPERIENCE_LOVE_COUNT = 'SHARE_EXPERIENCE.love_count',
  SHARE_EXPERIENCE_LIKE_BUTTON = 'SHARE_EXPERIENCE.like_button',
  SHARE_EXPERIENCE_LOVE_BUTTON = 'SHARE_EXPERIENCE.love_button',


  SHARE_EXPERIENCE_AREA_OF_EXPERTISE = 'SHARE_EXPERIENCE.area_of_expertise',
  SHARE_EXPERIENCE_AUTHOR = 'SHARE_EXPERIENCE.author',
  SHARE_EXPERIENCE_CONTENT = 'SHARE_EXPERIENCE.content',

  // CREATE EXPERIENCE
  CREATE_EXPERIENCE_TITLE = 'CREATE_EXPERIENCE.title',
  CREATE_EXPERIENCE_NAME_LABEL = 'CREATE_EXPERIENCE.name_label',
  CREATE_EXPERIENCE_NAME_PLACEHOLDER = 'CREATE_EXPERIENCE.name_placeholder',
  CREATE_EXPERIENCE_AREA_LABEL = 'CREATE_EXPERIENCE.area_label',
  CREATE_EXPERIENCE_AREA_PLACEHOLDER = 'CREATE_EXPERIENCE.area_placeholder',
  CREATE_EXPERIENCE_CONTENT_LABEL = 'CREATE_EXPERIENCE.content_label',
  CREATE_EXPERIENCE_CONTENT_PLACEHOLDER = 'CREATE_EXPERIENCE.content_placeholder',
  CREATE_EXPERIENCE_PREVIEW_LABEL = 'CREATE_EXPERIENCE.preview_label',
  CREATE_EXPERIENCE_SUBMIT = 'CREATE_EXPERIENCE.submit',
  CREATE_EXPERIENCE_ATTACHMENT_LABEL = 'CREATE_EXPERIENCE.attachment_label',
  CREATE_EXPERIENCE_ATTACHMENT_PLACEHOLDER = 'CREATE_EXPERIENCE.attachment_placeholder',
  CREATE_EXPERIENCE_ATTACHMENT_HINT = 'CREATE_EXPERIENCE.attachment_hint',

  // VALIDATION
  VALIDATION_REQUIRED = 'VALIDATION.required',
  VALIDATION_MIN_LENGTH = 'VALIDATION.min_length',

  // BUTTONS
  BUTTON_SUBMITTING = 'BUTTON.submitting',

  // CREATE EXPERIENCE ALERTS
  CREATE_EXPERIENCE_CONFIRM_SUBMIT_HEADER = 'CREATE_EXPERIENCE.confirm_submit_header',
  CREATE_EXPERIENCE_CONFIRM_SUBMIT_MESSAGE = 'CREATE_EXPERIENCE.confirm_submit_message',
  CREATE_EXPERIENCE_CONFIRM_CANCEL_HEADER = 'CREATE_EXPERIENCE.confirm_cancel_header',
  CREATE_EXPERIENCE_CONFIRM_CANCEL_MESSAGE = 'CREATE_EXPERIENCE.confirm_cancel_message',
  CREATE_EXPERIENCE_STAY_BUTTON = 'CREATE_EXPERIENCE.stay_button',
  CREATE_EXPERIENCE_EXIT_BUTTON = 'CREATE_EXPERIENCE.exit_button',

  // CREATE EXPERIENCE MESSAGES
  CREATE_EXPERIENCE_SUCCESS_MESSAGE = 'CREATE_EXPERIENCE.success_message',
  CREATE_EXPERIENCE_ERROR_AUTH = 'CREATE_EXPERIENCE.error_auth',
  CREATE_EXPERIENCE_ERROR_CREATE = 'CREATE_EXPERIENCE.error_create',

  // EMOTION STREAK
  EMOTION_STREAK_MONTHLY = 'EMOTION_STREAK.monthly',
  EMOTION_STREAK_DAYS_IN_MONTH = 'EMOTION_STREAK.days_in_month',

  // EMOTION SHARE
  EMOTION_SHARE_TARGET_CLASS = 'EMOTION_SHARE.target_class',
  EMOTION_SHARE_TARGET_FRIENDS = 'EMOTION_SHARE.target_friends',
  EMOTION_SHARE_TARGET_TEACHERS = 'EMOTION_SHARE.target_teachers',
  EMOTION_SHARE_TARGET_FAMILY = 'EMOTION_SHARE.target_family',

  // DAILY EMOTION JOURNAL
  DAILY_EMOTION_JOURNAL_NO_ENTRIES = 'DAILY_EMOTION_JOURNAL.no_entries',
  DAILY_EMOTION_JOURNAL_NO_CAPTION = 'DAILY_EMOTION_JOURNAL.no_caption',
  DAILY_EMOTION_JOURNAL_SHARED_WITH = 'DAILY_EMOTION_JOURNAL.shared_with',
  DAILY_EMOTION_JOURNAL_DATE_HAS_ENTRY = 'DAILY_EMOTION_JOURNAL.date_has_entry',
  DAILY_EMOTION_JOURNAL_ENTRY_CREATED = 'DAILY_EMOTION_JOURNAL.entry_created',
  DAILY_EMOTION_JOURNAL_ENTRY_CREATION_FAILED = 'DAILY_EMOTION_JOURNAL.entry_creation_failed',
  DAILY_EMOTION_JOURNAL_ENTRY_UPDATED = 'DAILY_EMOTION_JOURNAL.entry_updated',
  DAILY_EMOTION_JOURNAL_ENTRY_UPDATE_FAILED = 'DAILY_EMOTION_JOURNAL.entry_update_failed',
  DAILY_EMOTION_JOURNAL_ADD_ENTRY = 'DAILY_EMOTION_JOURNAL.add_entry',
  DAILY_EMOTION_JOURNAL_EDIT_ENTRY = 'DAILY_EMOTION_JOURNAL.edit_entry',
  DAILY_EMOTION_JOURNAL_SELECT_EMOTION = 'DAILY_EMOTION_JOURNAL.select_emotion',
  DAILY_EMOTION_JOURNAL_ADD_CAPTION = 'DAILY_EMOTION_JOURNAL.add_caption',
  DAILY_EMOTION_JOURNAL_CAPTION_PLACEHOLDER = 'DAILY_EMOTION_JOURNAL.caption_placeholder',
  DAILY_EMOTION_JOURNAL_SHARE_WITH = 'DAILY_EMOTION_JOURNAL.share_with',
  DAILY_EMOTION_JOURNAL_PUBLIC_SHARE = 'DAILY_EMOTION_JOURNAL.public_share',
  DAILY_EMOTION_JOURNAL_PUBLIC_SHARE_DESCRIPTION = 'DAILY_EMOTION_JOURNAL.public_share_description',
  DAILY_EMOTION_JOURNAL_SELECT_SHARE_TYPE = 'DAILY_EMOTION_JOURNAL.select_share_type',
  DAILY_EMOTION_JOURNAL_SELECT_USERS = 'DAILY_EMOTION_JOURNAL.select_users',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_ALL = 'DAILY_EMOTION_JOURNAL.share_type_all',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_ALL_DETAIL = 'DAILY_EMOTION_JOURNAL.share_type_all_detail',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_ALL_FRIENDS = 'DAILY_EMOTION_JOURNAL.share_type_all_friends',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_ALL_FRIENDS_DETAIL = 'DAILY_EMOTION_JOURNAL.share_type_all_friends_detail',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_FRIENDS = 'DAILY_EMOTION_JOURNAL.share_type_friends',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_FRIENDS_DETAIL = 'DAILY_EMOTION_JOURNAL.share_type_friends_detail',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_PARENT = 'DAILY_EMOTION_JOURNAL.share_type_parent',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_PARENT_DETAIL = 'DAILY_EMOTION_JOURNAL.share_type_parent_detail',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_TEACHER = 'DAILY_EMOTION_JOURNAL.share_type_teacher',
  DAILY_EMOTION_JOURNAL_SHARE_TYPE_TEACHER_DETAIL = 'DAILY_EMOTION_JOURNAL.share_type_teacher_detail',
  DAILY_EMOTION_JOURNAL_NO_USERS_FOUND = 'DAILY_EMOTION_JOURNAL.no_users_found',
  DAILY_EMOTION_JOURNAL_NO_ENTRY_FOR_DATE = 'DAILY_EMOTION_JOURNAL.no_entry_for_date',
  DAILY_EMOTION_JOURNAL_STOP_SHARING = 'DAILY_EMOTION_JOURNAL.stop_sharing',

  // CALENDAR
  CALENDAR_TODAY = 'CALENDAR.today',
  CALENDAR_SUNDAY_SHORT = 'CALENDAR.sunday_short',
  CALENDAR_MONDAY_SHORT = 'CALENDAR.monday_short',
  CALENDAR_TUESDAY_SHORT = 'CALENDAR.tuesday_short',
  CALENDAR_WEDNESDAY_SHORT = 'CALENDAR.wednesday_short',
  CALENDAR_THURSDAY_SHORT = 'CALENDAR.thursday_short',
  CALENDAR_FRIDAY_SHORT = 'CALENDAR.friday_short',
  CALENDAR_SATURDAY_SHORT = 'CALENDAR.saturday_short',
  CALENDAR_JANUARY = 'CALENDAR.january',
  CALENDAR_FEBRUARY = 'CALENDAR.february',
  CALENDAR_MARCH = 'CALENDAR.march',
  CALENDAR_APRIL = 'CALENDAR.april',
  CALENDAR_MAY = 'CALENDAR.may',
  CALENDAR_JUNE = 'CALENDAR.june',
  CALENDAR_JULY = 'CALENDAR.july',
  CALENDAR_AUGUST = 'CALENDAR.august',
  CALENDAR_SEPTEMBER = 'CALENDAR.september',
  CALENDAR_OCTOBER = 'CALENDAR.october',
  CALENDAR_NOVEMBER = 'CALENDAR.november',
  CALENDAR_DECEMBER = 'CALENDAR.december',

  // HOME
  HOME_WELCOME_TITLE = 'HOME.welcome_title',
  HOME_WELCOME_SUBTITLE = 'HOME.welcome_subtitle',

  // TASK
  TASK_SUBTITLE = 'TASK.subtitle',

  // RANK PAGE
  RANK_POSITION = 'RANK.position',
  RANK_NICKNAME = 'RANK.nickname',
  RANK_POINTS = 'RANK.points',
  RANK_POINTS_HEADER = 'RANK.points_header',
  BADGE_NEW = 'RANK.badge_new',

  // FAMILY CONFLICT SURVEY
  TITLE_FAMILY_CONFLICT_SURVEY = 'TITLE.family_conflict_survey',
  TITLE_FAMILY_CONFLICT_SURVEY_HISTORY = 'TITLE.family_conflict_survey_history',
  TITLE_FAMILY_CONFLICT_SURVEY_DETAIL = 'TITLE.family_conflict_survey_detail',
  TITLE_CONFLICT_LEVEL = 'TITLE.conflict_level',
  TITLE_SYSTEM_FEEDBACK = 'TITLE.system_feedback',
  TITLE_SURVEY_QUESTIONS = 'TITLE.survey_questions',
  TITLE_NO_SURVEY_HISTORY = 'TITLE.no_survey_history',
  TITLE_TAKE_SURVEY_NOW = 'TITLE.take_survey_now',

  // FAMILY COMMUNICATION QUALITY SURVEY
  TITLE_FAMILY_COMMUNICATION_QUALITY_SURVEY = 'TITLE.family_communication_quality_survey',
  TITLE_FAMILY_COMMUNICATION_QUALITY_SURVEY_HISTORY = 'TITLE.family_communication_quality_survey_history',
  TITLE_FAMILY_COMMUNICATION_QUALITY_SURVEY_DETAIL = 'TITLE.family_communication_quality_survey_detail',
  TITLE_COMMUNICATION_LEVEL = 'TITLE.communication_level',
  TITLE_FAMILY_COMMUNICATION_SURVEY_QUESTIONS = 'TITLE.family_communication_survey_questions',
  TITLE_COMMUNICATION_POINTS = 'TITLE.communication_points',
  TITLE_PAGE = 'TITLE.page',
  TITLE_NO_FAMILY_COMMUNICATION_SURVEY_HISTORY = 'TITLE.no_family_communication_survey_history',

  // SELF DISCOVERY SURVEY
  TITLE_SELF_DISCOVERY_SURVEY = 'TITLE.self_discovery_survey',
  TITLE_SELF_DISCOVERY_SURVEY_HISTORY = 'TITLE.self_discovery_survey_history',
  TITLE_SELF_DISCOVERY_SURVEY_DETAIL = 'TITLE.self_discovery_survey_detail',
  TITLE_DISCOVERY_LEVEL = 'TITLE.discovery_level',
  TITLE_SELF_DISCOVERY_SURVEY_QUESTIONS = 'TITLE.self_discovery_survey_questions',
  TITLE_DISCOVERY_POINTS = 'TITLE.discovery_points',
  TITLE_NO_SELF_DISCOVERY_SURVEY_HISTORY = 'TITLE.no_self_discovery_survey_history',

  // EMOTIONAL SURVEY
  TITLE_EMOTIONAL_SURVEY = 'TITLE.emotional_survey',
  TITLE_EMOTIONAL_SURVEY_HISTORY = 'TITLE.emotional_survey_history',
  TITLE_EMOTIONAL_SURVEY_DETAIL = 'TITLE.emotional_survey_detail',

  // FAMILY DIALOGUE SESSION
  TITLE_FAMILY_DIALOGUE_SESSION = 'TITLE.family_dialogue_session',
  TITLE_FAMILY_DIALOGUE_SESSION_HISTORY = 'TITLE.family_dialogue_session_history',
  TITLE_FAMILY_DIALOGUE_SESSION_DETAIL = 'TITLE.family_dialogue_session_detail',
  TITLE_FAMILY_DIALOGUE_QUESTIONS = 'TITLE.family_dialogue_questions',
  TITLE_SUGGESTED_QUESTIONS = 'TITLE.suggested_questions',
  TITLE_PARENT_QUESTIONS = 'TITLE.parent_questions',
  TITLE_NO_FAMILY_DIALOGUE_SESSION_HISTORY = 'TITLE.no_family_dialogue_session_history',
  TITLE_ANSWER_QUESTIONS = 'TITLE.answer_questions',

  // TITLE
  TITLE_UNSUPPORTED_QUESTION_TYPE = 'TITLE.unsupported_question_type',
  TITLE_PHONE = 'TITLE.phone',
  TITLE_ACCOUNT = 'TITLE.account',
  TITLE_EMAIL = 'TITLE.email',
  TITLE_PASSWORD = 'TITLE.password',
  TITLE_NEW_PASSWORD = 'TITLE.new_password',
  TITLE_CONFIRM_PASSWORD = 'TITLE.confirm_password',
  TITLE_LOGIN = 'TITLE.login',
  TITLE_LOGOUT = 'TITLE.logout',
  TITLE_REMEMBER_ME = 'TITLE.remember_me',
  TITLE_DATA_UPDATING = 'TITLE.data_updating',
  TITLE_INPUT_PIN = 'TITLE.input_pin',
  TITLE_AUTH_USER_TITLE = 'TITLE.auth_user_title',
  TITLE_AUTH_BIOMETRIC_UNLOCK_APP = 'TITLE.auth_biometric_unlock_app',
  TITLE_VERSION = 'TITLE.version',
  TITLE_HOME = 'TITLE.home',
  TITLE_PROFILE = 'TITLE.profile',
  TITLE_DASHBOARD = 'TITLE.dashboard',
  TITLE_CHATBOT = 'TITLE.chatbot',
  TITLE_RANK = 'TITLE.rank',
  TITLE_MENU = 'TITLE.menu',
  TITLE_DAYS = 'TITLE.days',
  TITLE_VIEW_PROFILE_PAGE = 'TITLE.view_profile_page',
  TITLE_BADGE_COLLECTION = 'TITLE.badge_collection',
  TITLE_FAMILY_PAGE = 'TITLE.family_page',
  TITLE_CLASS_GROUP = 'TITLE.class_group',
  TITLE_SCHOOL_GROUP = 'TITLE.school_group',
  TITLE_FRIENDS = 'TITLE.friends',
  TITLE_TOTAL_FRIENDS = 'TITLE.total_friends',
  TITLE_CLOSE_FRIENDS = 'TITLE.close_friends',
  TITLE_ADD_FRIEND = 'TITLE.add_friend',
  TITLE_SEARCH_FRIENDS = 'TITLE.search_friends',
  TITLE_SEARCH = 'TITLE.search',
  TITLE_DAILY_EMOTION_JOURNAL = 'TITLE.daily_emotion_journal',
  TITLE_CALENDAR = 'TITLE.calendar',
  TITLE_HISTORY = 'TITLE.history',
  TITLE_EMOTION_STREAK = 'TITLE.emotion_streak',
  TITLE_EMOTION_STREAK_MONTH = 'TITLE.emotion_streak_month',
  TITLE_EMOTION_CAPTION = 'TITLE.emotion_caption',
  TITLE_EMOTION_SHARE = 'TITLE.emotion_share',
  TITLE_EMOTION_SHARE_TARGET = 'TITLE.emotion_share_target',
  TITLE_ACHIEVEMENTS = 'TITLE.achievements',
  TITLE_YOUR_RANK = 'TITLE.your_rank',
  TITLE_YOUR_ACHIEVEMENTS = 'TITLE.your_achievements',
  TITLE_FRIEND_DETAIL = 'TITLE.friend_detail',
  TITLE_MESSAGE = 'TITLE.message',
  TITLE_UNFRIEND = 'TITLE.unfriend',
  TITLE_BADGES = 'TITLE.badges',
  TITLE_MISSIONS = 'TITLE.missions',
  TITLE_FRIENDLY = 'TITLE.friendly',
  TITLE_SCHOOL = 'TITLE.school',
  TITLE_CLASS = 'TITLE.class',
  TITLE_THEME_AND_BACKGROUND = 'TITLE.theme_and_background',
  TITLE_NOTIFICATION_AND_SOUND = 'TITLE.notification_and_sound',
  TITLE_LANGUAGE = 'TITLE.language',
  TITLE_ACCOUNT_AND_SECURITY = 'TITLE.account_and_security',
  TITLE_PRIVACY_RIGHTS = 'TITLE.privacy_rights',
  TITLE_REPORT_PROBLEMS = 'TITLE.report_problems',
  TITLE_CONTACT_AND_SUPPORT = 'TITLE.contact_and_support',
  TITLE_TERMS_AND_POLICIES = 'TITLE.terms_and_policies',
  TITLE_THEME = 'TITLE.theme',
  TITLE_THEME_LIGHT = 'TITLE.theme_light',
  TITLE_THEME_DARK = 'TITLE.theme_dark',
  TITLE_THEME_SYSTEM = 'TITLE.theme_system',
  TITLE_TEXT_SIZE = 'TITLE.text_size',
  TITLE_TEXT_SIZE_SMALL = 'TITLE.text_size_small',
  TITLE_TEXT_SIZE_MEDIUM = 'TITLE.text_size_medium',
  TITLE_TEXT_SIZE_LARGE = 'TITLE.text_size_large',
  TITLE_TEXT_SIZE_BIG_SIZE = 'TITLE.text_size_big_size',
  TITLE_AVATAR = 'TITLE.avatar',
  TITLE_BACKGROUND = 'TITLE.background',
  TITLE_DEFAULT = 'TITLE.default',
  TITLE_SOUND_EFFECT = 'TITLE.sound_effect',
  TITLE_BACKGROUND_SOUND = 'TITLE.background_sound',
  TITLE_TOUCH_SOUND = 'TITLE.touch_sound',
  TITLE_NOTIFICATION_SOUND = 'TITLE.notification_sound',
  TITLE_RELOAD_SOUND = 'TITLE.reload_sound',
  TITLE_LOADING_SOUND = 'TITLE.loading_sound',
  TITLE_MESSAGE_SOUND = 'TITLE.message_sound',
  TITLE_SUCCESS_SOUND = 'TITLE.success_sound',
  TITLE_FAILED_SOUND = 'TITLE.failed_sound',
  TITLE_MUSIC = 'TITLE.music',
  TITLE_NOTIFICATION = 'TITLE.notification',
  TITLE_EXPERT_GUIDE = 'TITLE.expert_guide',
  TITLE_EXPERT_GUIDE_DETAIL = 'TITLE.expert_guide_detail',
  TITLE_SHARE = 'TITLE.share',
  TITLE_ON = 'TITLE.on',
  TITLE_OFF = 'TITLE.off',
  TITLE_SHOW = 'TITLE.show',
  TITLE_HIDE = 'TITLE.hide',
  TITLE_BIRTHDAY = 'TITLE.birthday',
  TITLE_BIRTHDAY_NOTIFICATION = 'TITLE.birthday_notification',
  TITLE_LOCK_APP = 'TITLE.lock_app',
  TITLE_LOGIN_BY_BIOMETRIC = 'TITLE.login_by_biometric',
  TITLE_CHANGE_PASSWORD = 'TITLE.change_password',
  TITLE_PERSONAL_INFO = 'TITLE.personal_info',
  TITLE_SETUP_LOCK_APP_PIN = 'TITLE.setup_lock_app_pin',
  TITLE_CHANGE_LOCK_APP_PIN = 'TITLE.change_lock_app_pin',
  TITLE_AUTO_LOCK_APP = 'TITLE.auto_lock_app',
  TITLE_UNLOCK_APP_BIOMETRIC = 'TITLE.unlock_app_biometric',
  TITLE_ENABLE_VIEW_AND_COMMENT = 'TITLE.enable_view_and_comment',
  TITLE_BLOCK_AND_HIDDEN = 'TITLE.block_and_hidden',
  TITLE_AUTO_LOCK_IMMEDIATELY = 'TITLE.auto_lock_immediately',
  TITLE_AUTO_LOCK_30_SECONDS = 'TITLE.auto_lock_30_seconds',
  TITLE_AUTO_LOCK_1_MINUTE = 'TITLE.auto_lock_1_minute',
  TITLE_AUTO_LOCK_5_MINUTES = 'TITLE.auto_lock_5_minutes',
  TITLE_AUTO_LOCK_15_MINUTES = 'TITLE.auto_lock_15_minutes',
  TITLE_AUTO_LOCK_30_MINUTES = 'TITLE.auto_lock_30_minutes',
  TITLE_ENTER_NEW_PIN = 'TITLE.enter_new_pin',
  TITLE_CONFIRM_NEW_PIN = 'TITLE.confirm_new_pin',
  TITLE_ENTER_CURRENT_PIN = 'TITLE.enter_current_pin',
  TITLE_APP_UPDATE = 'TITLE.app_update',
  TITLE_NOT_SET = 'TITLE.not_set',
  TITLE_NOT_SET_NICKNAME = 'TITLE.not_set_nickname',
  TITLE_CHATBOT_AI = 'TITLE.chatbot_ai',
  TITLE_TYPE_MESSAGE = 'TITLE.type_message',
  TITLE_POINTS = 'TITLE.points',
  TITLE_LOADING_MORE = 'TITLE.loading_more',
  TITLE_ACTIONS = 'TITLE.actions',
  TITLE_NAME = 'TITLE.name',
  TITLE_GENDER = 'TITLE.gender',
  TITLE_MALE = 'TITLE.male',
  TITLE_FEMALE = 'TITLE.female',
  TITLE_OTHER = 'TITLE.other',
  TITLE_STATE = 'TITLE.state',
  TITLE_DISTRICT = 'TITLE.district',
  TITLE_PRECINT = 'TITLE.precint',
  TITLE_STREET = 'TITLE.street',
  TITLE_SETUP_NICKNAME = 'TITLE.setup_nickname',
  TITLE_IMAGE = 'TITLE.image',
  TITLE_TAKE_NEW_IMAGE = 'TITLE.take_new_image',
  TITLE_SELECT_GALLERY = 'TITLE.select_gallery',
  TITLE_TASK_ACTIVITY = 'TITLE.task_activity',
  TITLE_TASK = 'TITLE.task',
  TITLE_ACTIVITY = 'TITLE.activity',
  TITLE_CHECKIN_EMOTION_DAILY = 'TITLE.checkin_emotion_daily',
  TITLE_CHECKIN_EMOTION_NOW = 'TITLE.checkin_emotion_now',
  TITLE_HAS_SURVEY_PENDING = 'TITLE.has_survey_pending',
  TITLE_GROUP_CHALLENGE = 'TITLE.group_challenge',
  TITLE_FAMILY_ACTIVITY = 'TITLE.family_activity',
  TITLE_INSTRUCTION = 'TITLE.instruction',
  TITLE_EXERCISE = 'TITLE.exercise',
  TITLE_PERIODIC_SURVEY = 'TITLE.periodic_survey',
  TITLE_FRIEND_REQUESTS = 'TITLE.friend_requests',
  TITLE_NO_FRIENDS = 'TITLE.no_friends',
  TITLE_NO_FRIEND_REQUESTS = 'TITLE.no_friend_requests',
  TITLE_SEND_REQUEST = 'TITLE.send_request',
  TITLE_REQUEST_TO_YOU = 'TITLE.request_to_you',
  TITLE_NO_RESULT_FOUND = 'TITLE.no_result_found',
  TITLE_SELF_DISCOVERY_PROGRESS = 'TITLE.sel_discovery_progress',
  TITLE_NO_DATA = 'TITLE.no_data',
  TITLE_COMPLETED = 'TITLE.completed',
  TITLE_IN_PROGRESS = 'TITLE.in_progress',
  TITLE_PENDING = 'TITLE.pending',
  TITLE_IGNORE = 'TITLE.ignore',
  TITLE_PROGRESS_UPDATING = 'TITLE.progress_updating',
  TITLE_RESULT = 'TITLE.result',
  TITLE_PROGRESS_RESULT = 'TITLE.progress_result',
  TITLE_RESULT_DESCRIPTION = 'TITLE.result_description',
  TITLE_CAPTURE = 'TITLE.capture',
  TITLE_GALLERY = 'TITLE.gallery',
  TITLE_DOCUMENTS = 'TITLE.documents',
  TITLE_TEENAGER_SHARED_EMOTION_DIARY = 'TITLE.teenager_shared_emotion_diary',
  TITLE_NEW_NOTIFICATION = 'TITLE.new_notification',
  TITLE_NEW_NOTIFICATION_DETAIL = 'TITLE.new_notification_detail',
  TITLE_GROUP_TASKS = 'TITLE.group_tasks',
  TITLE_GROUP_TASKS_DETAIL = 'TITLE.group_tasks_detail',
  TITLE_EXPERT_GUIDE_DETAILS = 'TITLE.expert_guide_details',
  TITLE_CREATE_GROUP_TASKS = 'TITLE.create_group_task',
  TITLE_TASK_NAME = 'TITLE.task_name',
  TITLE_TASK_GUIDE = 'TITLE.task_guide',

  // VALIDATE
  VALIDATE_PHONE_REQUIRED = 'VALIDATE.phone_required',
  VALIDATE_ACCOUNT_REQUIRED = 'VALIDATE.account_required',
  VALIDATE_PASS_REQUIRED = 'VALIDATE.password_required',
  VALIDATE_NEW_PASS_REQUIRED = 'VALIDATE.new_password_required',
  VALIDATE_CONFIRM_PASS_REQUIRED = 'VALIDATE.confirm_password_required',
  VALIDATE_PASS_MIN_LENGTH = 'VALIDATE.password_min_length',
  VALIDATE_PASS_MATCH = 'VALIDATE.passwords_not_match',
  VALIDATE_NICKNAME_REQUIRED = 'VALIDATE.nickname_required',
  VALIDATE_NAME_REQUIRED = 'VALIDATE.name_required',
  VALIDATE_EMAIL_REQUIRED = 'VALIDATE.email_required',
  VALIDATE_EMAIL_FORMAT = 'VALIDATE.email_format',

  // ALERT
  ALERT_DEFAULT_HEADER = 'ALERT.default_header',
  ALERT_ERROR_HEADER = 'ALERT.error_header',
  ALERT_ERROR_SYSTEM_HEADER = 'ALERT.error_system_header',
  ALERT_ACCESS_PERMISSION_NOTIFICATION = 'ALERT.access_permission_notification_alert',
  ALERT_ACCESS_PERMISSION_GALLERY = 'ALERT.access_permission_gallery_alert',
  ALERT_CONFIRM_LOGOUT = 'ALERT.confirm_logout_alert',
  ALERT_CONFIRM_DELETE = 'ALERT.confirm_delete_alert',
  ALERT_CONFIRM_UNFRIEND = 'ALERT.confirm_unfriend_alert',
  ALERT_APP_UPDATE_SUCCESSFUL = 'ALERT.app_update_successful',
  ALERT_CONFIRM_UN_PUBLIC = 'ALERT.confirm_un_public_alert',
  ALERT_CONFIRM_END_TASK = 'ALERT.confirm_end_task',
  ALERT_CONFIRM_DELETE_TASK = 'ALERT.confirm_delete_task',
  ALERT_FEATURE_DEVELOPING = 'ALERT.feature_developing',

  // BUTTON TITLES
  BUTTON_CONFIRM = 'BUTTON_TITLES.confirm',
  BUTTON_CANCEL = 'BUTTON_TITLES.cancel',
  BUTTON_OPEN_SETTING = 'BUTTON_TITLES.open_settings',
  BUTTON_OK = 'BUTTON_TITLES.ok',
  BUTTON_CLOSE = 'BUTTON_TITLES.close',
  BUTTON_BACK = 'BUTTON_TITLES.back',
  BUTTON_CURRENT = 'BUTTON_TITLES.current',
  BUTTON_SET_AS_CURRENT = 'BUTTON_TITLES.set_as_current',
  BUTTON_UPDATE = 'BUTTON_TITLES.update',
  BUTTON_END = 'BUTTON_TITLES.end',
  BUTTON_EXECUTE = 'BUTTON_TITLES.execute',
  BUTTON_COMPLETE = 'BUTTON_TITLES.complete',
  BUTTON_CREATE_NEW_SURVEY = 'BUTTON_TITLES.create_new_survey',
  BUTTON_CREATE_FIRST_SURVEY = 'BUTTON_TITLES.create_first_survey',
  BUTTON_SELECT = 'BUTTON_TITLES.select',
  BUTTON_SHARE = 'BUTTON_TITLES.share',
  BUTTON_CHECKIN_EMOTION = 'BUTTON_TITLES.checkin_emotion',
  BUTTON_ACCEPT = 'BUTTON_TITLES.accept',
  BUTTON_SEND_REQUEST = 'BUTTON_TITLES.send_request',
  BUTTON_EXECUTING = 'BUTTON_TITLES.executing',
  BUTTON_NO_EXECUTION = 'BUTTON_TITLES.no_execution',
  BUTTON_IGNORE = 'BUTTON_TITLES.ignore',
  BUTTON_SUBMIT = 'BUTTON_TITLES.submit',
  BUTTON_DELETE = 'BUTTON_TITLES.delete',
  BUTTON_EDIT = 'BUTTON_TITLES.edit',
  BUTTON_DETAIL = 'BUTTON_TITLES.detail',

  // TOAST
  TOAST_ERROR_HEADER = 'TOAST.error_header',
  TOAST_SUCCESS_HEADER = 'TOAST.success_header',
  TOAST_INFO_HEADER = 'TOAST.info_header',
  TOAST_WARNING_HEADER = 'TOAST.warning_header',
  TOAST_AUTH_FAILED = 'TOAST.auth_failed',
  TOAST_AUTH_BY_PASSWORD = 'TOAST.auth_by_password',
  TOAST_NETWORK_ONLINE = 'TOAST.network_online',
  TOAST_NETWORK_OFFLINE = 'TOAST.network_offline',
  TOAST_CHANGE_PASS_SUCCESS = 'TOAST.password_changed_success',
  TOAST_TASK_COMPLETE_SUCCESS = 'TOAST.task_complete_success',
  TOAST_TASK_COMPLETE_ERROR = 'TOAST.task_complete_error',
  TOAST_UPDATE_FAILED = 'TOAST.update_failed',
  TOAST_UPDATE_SUCCESS = 'TOAST.update_success',
  TOAST_ANSWER_ALL_QUESTIONS = 'TOAST.answer_all_questions',
  TOAST_SEND_REQUEST_FAILED = 'TOAST.send_request_failed',
  TOAST_SEND_REQUEST_SUCCESS = 'TOAST.send_request_success',
  TOAST_CANCEL_REQUEST_SUCCESS = 'TOAST.cancel_request_success',
  TOAST_CANCEL_REQUEST_FAILED = 'TOAST.cancel_request_failed',
  TOAST_ACCEPT_REQUEST_SUCCESS = 'TOAST.accept_request_success',
  TOAST_ACCEPT_REQUEST_FAILED = 'TOAST.accept_request_failed',
  TOAST_CHECKIN_EMOTION_FAILED = 'TOAST.checkin_emotion_failed',
  TOAST_CHECKIN_EMOTION_SUCCESS = 'TOAST.checkin_emotion_success',
  TOAST_SEND_EMOTION_TIPS_SUCCESS = 'TOAST.send_emotion_tops_success',
  TOAST_SEND_EMOTION_TIPS_FAILED = 'TOAST.send_emotion_tops_failed',
  TOAST_CREATE_GROUP_TASK_SUCCESS = 'TOAST.create_group_task_success',
  TOAST_CREATE_GROUP_TASK_FAILED = 'TOAST.create_group_task_failed',
  TOAST_SEND_NOTIFICATION_SUCCESS = 'TOAST.send_notification_success',

  // ERROR
  ERROR_NETWORK = 'ERROR.network',
  ERROR_BAD_REQUEST = 'ERROR.bad_request',
  ERROR_UNAUTHORIZED = 'ERROR.unauthorized',
  ERROR_FORBIDDEN = 'ERROR.forbidden',
  ERROR_NOT_FOUND = 'ERROR.not_found',
  ERROR_SERVER = 'ERROR.server',
  ERROR_UNKNOWN = 'ERROR.unknown',
  ERROR_SERVER_CONNECTION = 'ERROR.server_connection',
  ERROR_INVALID_CURRENT_PASS = 'ERROR.invalid_current_password',
  ERROR_INVALID_EDIT_MODE = 'ERROR.invalid_edit_mode',
  ERROR_NO_EMOTION_SELECTED = 'ERROR.no_emotion_selected',
  ERROR_NO_AUTH_DATA = 'ERROR.no_auth_data',
  ERROR_CAN_ONLY_CREATE_TODAY = 'ERROR.can_only_create_today',

  // WARNING
  WARNING_APP_LOCK = 'WARNING.app_lock',

  // RESOURCE
  RESOURCE_LIBRARY = 'RESOURCE.library',
  RESOURCE_TITLE = 'RESOURCE.title',
  RESOURCE_DESCRIPTION = 'RESOURCE.description',
  RESOURCE_TYPE = 'RESOURCE.type',
  RESOURCE_TOPIC = 'RESOURCE.topic',
  RESOURCE_SEARCH = 'RESOURCE.search',
  RESOURCE_FILTER = 'RESOURCE.filter',
  RESOURCE_BACK_TO_LIST = 'RESOURCE.back_to_list',
  RESOURCE_LOADING = 'RESOURCE.loading',
  RESOURCE_TYPE_DOCUMENT = 'RESOURCE.type_document',
  RESOURCE_TYPE_VIDEO = 'RESOURCE.type_video',
  RESOURCE_TYPE_ALL = 'RESOURCE.type_all',
  RESOURCE_TOPIC_MUSIC = 'RESOURCE.topic_music',
  RESOURCE_TOPIC_EDUCATION = 'RESOURCE.topic_education',
  RESOURCE_TOPIC_SCIENCE = 'RESOURCE.topic_science',
  RESOURCE_TOPIC_TECHNOLOGY = 'RESOURCE.topic_technology',
  RESOURCE_TOPIC_ARTS = 'RESOURCE.topic_arts',
  RESOURCE_TOPIC_SPORTS = 'RESOURCE.topic_sports',
  RESOURCE_TOPIC_ALL = 'RESOURCE.topic_all',
  RESOURCE_VIDEO_LIBRARY = 'RESOURCE.video_library',
  RESOURCE_DOCUMENT_LIBRARY = 'RESOURCE.document_library',
  RESOURCE_NOT_FOUND = 'RESOURCE.resource_not_found',
  RESOURCE_NO_RESOURCES_FOUND = 'RESOURCE.no_resources_found',
  RESOURCE_LOADING_MORE = 'RESOURCE.loading_more',
  RESOURCE_SEARCH_PLACEHOLDER = 'RESOURCE.search_placeholder',

  // CAPTION
  CAPTION_CHATBOT = 'CAPTION.chatbot',

  // EXPERT GUIDE
  EXPERT_GUIDE = 'EXPERT_GUIDE.title',
  EXPERT_GUIDE_LIST_SUBTITLE = 'EXPERT_GUIDE.list_subtitle',
  EXPERT_GUIDE_CATEGORIES = 'EXPERT_GUIDE.categories',
  EXPERT_GUIDE_SCORES = 'EXPERT_GUIDE.scores',
  EXPERT_GUIDE_RANK_POINT = 'EXPERT_GUIDE.rank_point',
  EXPERT_GUIDE_DESCRIPTION = 'EXPERT_GUIDE.description',
  EXPERT_GUIDE_CONTENT = 'EXPERT_GUIDE.content',
  EXPERT_GUIDE_NO_DATA = 'EXPERT_GUIDE.no_data',
  EXPERT_GUIDE_SEARCH_PLACEHOLDER = 'EXPERT_GUIDE.search_placeholder',
  EXPERT_GUIDE_FILTER_ALL = 'EXPERT_GUIDE.filter_all',
  EXPERT_GUIDE_FILTER_AREA = 'EXPERT_GUIDE.filter_area',

  // PARENT DASHBOARD
  PARENT_DASHBOARD_TITLE = 'PARENT_DASHBOARD.title',
  PARENT_DASHBOARD_SELECT_CHILD = 'PARENT_DASHBOARD.select_child',
  PARENT_DASHBOARD_EMOTIONS = 'PARENT_DASHBOARD.emotions',
  PARENT_DASHBOARD_NEW_NOTIFICATIONS = 'PARENT_DASHBOARD.new_notifications',
  PARENT_DASHBOARD_FAMILY_ACTIVITIES = 'PARENT_DASHBOARD.family_activities',
  PARENT_DASHBOARD_EXPERT_SKILLS = 'PARENT_DASHBOARD.expert_skills',
  PARENT_DASHBOARD_VIEW_ALL = 'PARENT_DASHBOARD.view_all',
  PARENT_DASHBOARD_NO_CHILDREN = 'PARENT_DASHBOARD.no_children',
  PARENT_DASHBOARD_NO_EMOTIONS = 'PARENT_DASHBOARD.no_emotions',
  PARENT_DASHBOARD_NO_NOTIFICATIONS = 'PARENT_DASHBOARD.no_notifications',
  PARENT_DASHBOARD_NO_ACTIVITIES = 'PARENT_DASHBOARD.no_activities',
  PARENT_DASHBOARD_NO_EXPERT_GUIDES = 'PARENT_DASHBOARD.no_expert_guides',
  PARENT_DASHBOARD_PARENT_ROLE = 'PARENT_DASHBOARD.parent_role',
  PARENT_DASHBOARD_CHILDREN = 'PARENT_DASHBOARD.children',
  PARENT_DASHBOARD_EMOTIONS_DETAIL = 'PARENT_DASHBOARD.emotions_detail',
  PARENT_DASHBOARD_NOTIFICATIONS_DETAIL = 'PARENT_DASHBOARD.notifications_detail',
  PARENT_DASHBOARD_FAMILY_ACTIVITIES_DETAIL = 'PARENT_DASHBOARD.family_activities_detail',
  PARENT_DASHBOARD_EXPERT_SKILLS_DETAIL = 'PARENT_DASHBOARD.expert_skills_detail',

  LOADING_MORE = 'LOADING.loading_more',

  // CONFLICT LEVEL
  CONFLICT_LEVEL_LOW = 'CONFLICT_LEVEL.low',
  CONFLICT_LEVEL_MEDIUM = 'CONFLICT_LEVEL.medium',
  CONFLICT_LEVEL_HIGH = 'CONFLICT_LEVEL.high',
  CONFLICT_LEVEL_UNKNOWN = 'CONFLICT_LEVEL.unknown',

  // CLASS MANAGEMENT
  CLASS_MANAGEMENT_TITLE = 'CLASS_MANAGEMENT.title',
  CLASS_MANAGEMENT_NO_CLASSES = 'CLASS_MANAGEMENT.no_classes',
  CLASS_MANAGEMENT_CLASS_CODE = 'CLASS_MANAGEMENT.class_code',
  CLASS_MANAGEMENT_GRADE = 'CLASS_MANAGEMENT.grade',
  CLASS_MANAGEMENT_ACADEMIC_YEAR = 'CLASS_MANAGEMENT.academic_year',
  CLASS_MANAGEMENT_STUDENT_DETAIL = 'CLASS_MANAGEMENT.student_detail',
  CLASS_MANAGEMENT_STUDENT_INFO = 'CLASS_MANAGEMENT.student_info',
  CLASS_MANAGEMENT_STUDENT_CODE = 'CLASS_MANAGEMENT.student_code',
  CLASS_MANAGEMENT_STUDENTS = 'CLASS_MANAGEMENT.students',
  CLASS_MANAGEMENT_STUDENT_LIST = 'CLASS_MANAGEMENT.student_list',
  CLASS_MANAGEMENT_GENDER = 'CLASS_MANAGEMENT.gender',
  CLASS_MANAGEMENT_TOTAL_SCORE = 'CLASS_MANAGEMENT.total_score',
  CLASS_MANAGEMENT_CLASS_RANK = 'CLASS_MANAGEMENT.class_rank',
  CLASS_MANAGEMENT_NO_STUDENTS = 'CLASS_MANAGEMENT.no_students',

  // CONFLICT FEEDBACK
  CONFLICT_FEEDBACK_LOW = 'CONFLICT_FEEDBACK.low',
  CONFLICT_FEEDBACK_MEDIUM = 'CONFLICT_FEEDBACK.medium',
  CONFLICT_FEEDBACK_HIGH = 'CONFLICT_FEEDBACK.high',
  CONFLICT_FEEDBACK_UNKNOWN = 'CONFLICT_FEEDBACK.unknown',
}
